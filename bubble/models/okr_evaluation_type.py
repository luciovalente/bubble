...
PROMPT = """
    ...
    Se ci sono riferimenti al tipo di OKR usa il campo type.
...
class OkrEvaluationType(models.Model):
    ...
    def _check_python_code(self):
        for action in self.sudo().filtered("code"):
            ...
            if msg:
                text = (
                    " Error in your code. If you are using ChatGPT ask him again\n: %s"
                    % msg
                )
    ...
    def suggest_code_from_chatgpt(self):
        ...
        prompt = PROMPT % (
            self.description,
            self.get_library_and_variable(),
            self.get_model_and_fields(),
        )
        data = {
            "model": "gpt-4",
            "messages": [
